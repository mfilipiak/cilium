...
I'm working on the CRD part to try to wire in the MAC to the ARP parser.
cegp_types.go has the CiliumEgressGateway type
policy.go has the internal CiliumEgressGatewayPolicy

* I think i should try to figure out how to fix the 1.1.1.1 code and the tunnelendpoint ip in encap.h's refresh block
